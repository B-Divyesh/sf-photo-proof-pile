...
function landing() {
  ...
  const downloadControl = isMobileDevice()
    ? `<p class="mobile-download-note">Open this page on a desktop computer to check signed downloads.</p>`
...
async function showDownloads() {
  ...
  const dialog = document.createElement("dialog"); dialog.innerHTML = `<div class="download-dialog"><button class="dialog-close" aria-label="Close download window">×</button><p class="eyebrow">Desktop app</p><h2>Signed desktop downloads</h2><p id="release-state">Checking the latest release…</p><div id="release-links"></div><p class="fine" id="signature-state">Checking package signatures…</p></div>`; document.body.append(dialog); dialog.showModal(); dialog.querySelector(".dialog-close")?.addEventListener("click", () => dialog.close()); dialog.addEventListener("close", () => dialog.remove());
  try {
  ...
  } catch { document.querySelector("#release-state")!.textContent = "Signed downloads are not published yet. Check again later."; document.querySelector("#signature-state")!.textContent = "No package was offered because signature status could not be checked."; }
}
