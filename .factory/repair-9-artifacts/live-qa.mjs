...
const output = ".factory/repair-9-artifacts";
...
assert(await page.getByText("macOS and Windows builds are unsigned. Your system will ask you to confirm the first launch.").isVisible(), "unsigned package disclosure is missing");
assert(await page.locator("dialog a").count() === 4, "the complete release did not expose four platform downloads");
await page.screenshot({ path: `${output}/live-download-gate.png`, fullPage: true });
report.checks.downloadGate = { linksExposed: 4, publicState: "v0.1.17", signingDisclosure: "macOS and Windows unsigned" };
...
const notFoundResponse = await notFoundPage.goto(`${base}/not-a-proof-pile-route`, { waitUntil: "networkidle" });
await notFoundPage.locator("html").evaluate(element => { element.style.fontSize = "34px"; });
const notFoundWidth = await notFoundPage.evaluate(() => document.documentElement.scrollWidth);
assert(notFoundResponse?.status() === 404, `standalone 404 returned ${notFoundResponse?.status()}`);
assert(notFoundWidth <= 390, `standalone 404 overflowed at 200% text: ${notFoundWidth}px`);
const notFoundAxe = await new AxeBuilder({ page: notFoundPage }).analyze();
assert(notFoundAxe.violations.filter(item => ["serious", "critical"].includes(item.impact ?? "")).length === 0, "standalone 404 axe serious/critical violation");
await notFoundPage.screenshot({ path: `${output}/live-404-mobile-200.png`, fullPage: true });
report.checks.notFoundMobile200 = { status: 404, viewport: 390, documentWidth: notFoundWidth, axe: "zero serious or critical findings" };
...
assert(await mobile.getByText("Open this page on a desktop computer to check downloads.").isVisible(), "mobile download guidance is missing");
assert(await mobile.getByRole("button", { name: /Check download/ }).count() === 0, "mobile exposed a misleading download button");
