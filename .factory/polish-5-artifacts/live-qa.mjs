...
const output = ".factory/polish-5-artifacts";
...
await page.getByText("Signed downloads are not published yet. Check again later.").waitFor();
assert(await page.getByText("No package was offered because signature status could not be checked.").isVisible(), "signature refusal is missing");
assert(await page.locator("dialog a").count() === 0, "an unverified download link was exposed");
await page.screenshot({ path: `${output}/live-download-gate.png`, fullPage: true });
report.checks.downloadGate = { linksExposed: 0, publicState: "no release", signatureRefusal: true };
...
assert(await mobile.getByRole("button", { name: /Check signed download/ }).count() === 0, "mobile exposed a misleading download button");
