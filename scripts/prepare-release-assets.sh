...
jq -n --arg version "$RELEASE_TAG" --arg commit "$RELEASE_COMMIT" --arg base "$download_base" --argjson macos "$macos" --argjson windows "$windows" --argjson linux "$linux" \
  '{version: $version, commit: $commit, signatures: {macos: "no_developer_id", windows: "not_signed"}, platforms: {macos: ($macos | map({name: ., url: ($base + "/" + .)})), windows: ($windows | map({name: ., url: ($base + "/" + .)})), linux: ($linux | map({name: ., url: ($base + "/" + .)}))}}' > latest.json
jq -e --arg tag "$RELEASE_TAG" --arg commit "$RELEASE_COMMIT" '.version == $tag and .commit == $commit and .signatures.macos == "no_developer_id" and .signatures.windows == "not_signed" and (.platforms.macos | length) >= 2 and (.platforms.windows | length) >= 1 and (.platforms.linux | length) >= 2' latest.json
