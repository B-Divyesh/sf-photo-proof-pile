...
function runInstaller(checksum: string, trusted = true, releaseAvailable = true, markerBody = '{"macos":"signed-and-notarized","windows":"authenticode-signed"}') {
...
case "$url" in
  ...
  *DESKTOP_SIGNATURES_VERIFIED.json) printf '%s' '${markerBody}' > "$out" ;;
  *) exit 1 ;;
esac
`;
...
test("@claim:installer-checksum installs only a package matching SHA256SUMS", () => {
  ...
  const invalidMarker = runInstaller(createHash("sha256").update(payload).digest("hex"), true, true, '{"macos":"unsigned","windows":"unsigned"}');
  try {
    expect(invalidMarker.result.status).toBe(1);
    expect(invalidMarker.result.stderr).toContain("Desktop signature verification is incomplete");
    expect(existsSync(invalidMarker.target)).toBe(false);
  } finally { rmSync(invalidMarker.root, { recursive: true, force: true }); }
